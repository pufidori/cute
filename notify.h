...
public:
    ...
    void think() {
        int y{ 5 };
        int size{ render::logevent.m_size.m_height + 1 };
        ...
        for (size_t i{}; i < m_notify_text.size(); ++i) {
            ...
            render::FontSize_t text_size = render::logevent.size(notify->m_text);
            render::FontSize_t prefix_size = render::logevent.size("[cute] ");
            ...
            render::logevent.string(x, y, colors::sakso, "[cute] ");
            render::logevent.string(x + prefix_size.m_width + 1, y, color, notify->m_text);
