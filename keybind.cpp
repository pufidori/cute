...
void Keybind::draw() {
	...
	// box outline.
	render::rect(p.x + KEYBIND_X_OFFSET, p.y + 15, m_w - KEYBIND_X_OFFSET, KEYBIND_BOX_HEIGHT, { 25, 25, 25, m_parent->m_alpha });

	// inner blob
	// todo; gradient?
	render::rect_filled(p.x + KEYBIND_X_OFFSET + 1, p.y + 15 + 1, m_w - KEYBIND_X_OFFSET - 2, KEYBIND_BOX_HEIGHT - 2, { 25, 25, 25, m_parent->m_alpha });

	// todo; animate with '...'
	if (m_set)
		render::menu.string(p.x + KEYBIND_X_OFFSET + KEYBIND_ITEM_X_OFFSET, p.y + 15 + 4, color, XOR("..."));
