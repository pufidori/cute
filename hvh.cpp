...
void HVH::DoRealAntiAim() {
		...
		// run normal aa code.
			...
			case 5: {
				...
				break;
				  ...
				  // alicespin.
			/*case 6: {
				float m_alice_range = (g_menu.main.antiaim.alice_range_stand.get());
				float m_alice_speed = (g_menu.main.antiaim.alice_speed_stand.get());
				...
				static float last_time = 0.0f; // store the last time the function was called

				// get the current time
				float cur_time = g_csgo.m_globals->m_curtime;

				// initialize the angle and last_time only on the first call
				if (last_time == 0.0f) {
					current_angle = m_direction - m_alice_range;
					last_time = cur_time;
				}

				// calculate the time difference since the last call
				float time_diff = cur_time - last_time;
				last_time = cur_time;

				// calculate the angle increment based on rotation speed
				float angle_increment = m_alice_speed * 15.f * time_diff;

				// update the angle based on the current direction
				if (rotate_right) {
					current_angle += angle_increment;
					if (current_angle >= (m_direction + m_alice_range)) {
						// if it reaches the upper limit, change direction
						current_angle = (m_direction + m_alice_range);
						rotate_right = false;
					}
				}
					...
					current_angle -= angle_increment;
					if (current_angle <= (m_direction - m_alice_range)) {
						// if it reaches the lower limit, change direction
						current_angle = (m_direction - m_alice_range);
						rotate_right = true;
					}
				}
	...
	if (trace.m_fraction != 1.f && trace.m_plane.m_normal.z > 0.7f && !(g_cl.m_flags & FL_ONGROUND)) {
		...
		// Set landing time and flag.
		g_cl.m_landing_time = std::chrono::steady_clock::now();
		g_cl.m_landed_recently = true;
	}

	// If the player landed recently, check if 1 second has passed.
	if (g_cl.m_landed_recently) {
		auto now = std::chrono::steady_clock::now();
		auto duration_since_landing = std::chrono::duration_cast<std::chrono::milliseconds>(now - g_cl.m_landing_time).count();

		if (duration_since_landing <= g_menu.main.antiaim.landlen.get() && !(g_cl.m_buttons & IN_JUMP)) {
			// Call pitch for a certain duration after landing
			g_cl.m_cmd->m_view_angles.x = g_menu.main.antiaim.landangle.get();
		}
		else {
			// Reset the flag after 1 second.
			g_cl.m_landed_recently = false;
		}
	}
...
void HVH::AntiAim() {
	...
	if (m_mode == AntiAimMode::STAND) {
		...
		m_alice_range = g_menu.main.antiaim.alice_range_stand.get();
		m_alice_speed = g_menu.main.antiaim.alice_speed_stand.get();
...
void HVH::SendPacket() {
	...
	if (trace.m_fraction != 1.f && trace.m_plane.m_normal.z > 0.7f && !(g_cl.m_flags & FL_ONGROUND)) {
		...
		if (g_menu.main.antiaim.fakelag_reset_bhop.get() ||
			(g_menu.main.antiaim.body_yaw_fake.get() && g_csgo.RandomInt(0, 100) < 5)) {
			*g_cl.m_packet = true;
		}
	}
	...
	//"lagpeek"
	if ((g_cl.m_buttons & IN_USE) && g_cl.m_flags & FL_ONGROUND) {
		*g_cl.m_packet = g_cl.m_lag >= 3;
	}
