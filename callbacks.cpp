...
void callbacks::ConfigLoad1() {
	g_config.load(&g_menu.main, XOR("1.vip"));
...
void callbacks::ConfigLoad() {
	...
	std::string file = tfm::format(XOR("%s.vip"), config.data());
...
bool callbacks::distortion() {
	return g_menu.main.antiaim.yaw_stand.get() == 7;
	//return g_menu.main.antiaim.mode.get() == 0;
}
