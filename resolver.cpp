...
void Resolver::LastMoveLby(LagRecord* record, AimPlayer* data, Player* player)
{
	...
	if (!data->m_moved) {
		...
		*/
		...
		if (data->m_body != data->m_body_proxy_old)
		{
			record->m_eye_angles.y = record->m_body;

			data->m_body_updated_idk = record->m_anim_time + 1.125f;
...
void Resolver::ResolveStand(AimPlayer* data, LagRecord* record) {
	...
	if (data->m_moved) {
		...
		// LBY SHOULD HAVE UPDATED HERE.
		else if (record->m_anim_time >= data->m_body_updated_idk) {
			// only shoot the LBY flick 3 times.
			// if we happen to miss then we most likely mispredicted.
			if (data->m_body_idx < 1) {
				// set angles to current LBY.
				...
				data->m_body_updated_idk = record->m_anim_time + 1.1f;
...
void Resolver::ResolveAir(AimPlayer* data, LagRecord* record, Player* player) {
	...
	if (record->m_velocity.length_2d() < 60.f) {
		...
		// so the shot parsing won't pick the hits/misses up.
	...
	// calculate the angle difference between the view angle and velocity direction
	float delta = std::abs(ballz::AngleDiff(player->m_angEyeAngles().y, velyaw));

	// modify eye angles based on the number of shots and angle difference
	...
	case 3:
		...
		break;
	case 4:
		record->m_eye_angles.y = velyaw + (delta < 45.f ? 45.f : -45.f);
		break;
	case 5:
		record->m_eye_angles.y = velyaw - (delta < 45.f ? 45.f : -45.f);
		break;
...
bool Resolver::lby_updated(LagRecord* a, AimPlayer* b, Player* entity) {
	...
	if (b->m_body != b->m_body_proxy_old) return true;
